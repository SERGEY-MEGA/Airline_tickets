-- LAB2: тестовые данные для PostgreSQL.
-- В LAB2 таблицы создаёт Hibernate, а этот файл заполняет их после создания схемы.

-- Тестовые рейсы для быстрой проверки в Postman.
...
-- Тестовые бронирования: по одному на каждый стартовый рейс.
INSERT INTO bookings (id, flight_id, passenger_id, service_class, seat) VALUES
    (1, 1, 1, 'ECONOMY', '1A'),
    (2, 2, 2, 'BUSINESS', '2B'),
    (3, 3, 3, 'ECONOMY', '3C');

-- После ручных id сдвигаем identity-счётчики, чтобы новые записи не конфликтовали с тестовыми.
